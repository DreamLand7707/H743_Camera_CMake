...
static inline uint8_t SCCB_Read16(uint8_t slv_addr, const uint16_t reg) {
    uint8_t ret = 0;
    OV5640_RD_Reg(&my_sccb, reg, &ret);
    return ret;
}

static inline int SCCB_Write16(uint8_t slv_addr, uint16_t reg, uint8_t data) {
    return OV5640_WR_Reg(&my_sccb, reg, data);
}
...
static int calc_sysclk(int xclk, bool pll_bypass, int pll_multiplier, int pll_sys_div, int pre_div, bool root_2x, int pclk_root_div, bool pclk_manual, int pclk_div) {
    ...
    unsigned int REFIN             = (xclk / (unsigned int)pll_pre_div);
...
static int set_framesize(sensor_t *sensor, framesize_t framesize) {
    ...
    ret                       = write_addr_reg(sensor->slv_addr, X_ADDR_ST_H, settings.start_x, settings.start_y) || write_addr_reg(sensor->slv_addr, X_ADDR_END_H, settings.end_x, settings.end_y) || write_addr_reg(sensor->slv_addr, X_OUTPUT_SIZE_H, w, h);
    ...
    if (!sensor->status.binning) {
        ret = write_addr_reg(sensor->slv_addr, X_TOTAL_SIZE_H, settings.total_x, settings.total_y) || write_addr_reg(sensor->slv_addr, X_OFFSET_H, settings.offset_x, settings.offset_y);
    }
    else {
        if (w > 920) {
...
static int set_res_raw(sensor_t *sensor, int startX, int startY, int endX, int endY, int offsetX, int offsetY, int totalX, int totalY, int outputX, int outputY, bool scale, bool binning) {
    int ret = 0;
    ret     = write_addr_reg(sensor->slv_addr, X_ADDR_ST_H, startX, startY) || write_addr_reg(sensor->slv_addr, X_ADDR_END_H, endX, endY) || write_addr_reg(sensor->slv_addr, X_OFFSET_H, offsetX, offsetY) || write_addr_reg(sensor->slv_addr, X_TOTAL_SIZE_H, totalX, totalY) || write_addr_reg(sensor->slv_addr, X_OUTPUT_SIZE_H, outputX, outputY) || write_reg_bits(sensor->slv_addr, ISP_CONTROL_01, 0x20, scale);
    if (!ret) {
...
static int init_status(sensor_t *sensor) {
    ...
    sensor->status.sharpness      = (int8_t)((read_reg(sensor->slv_addr, 0x5303) / 8) - 3);
